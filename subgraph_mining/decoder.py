...
def pattern_growth_streaming(dataset, task, args):
    graph = dataset[0]
    graph_chunks = process_large_graph_in_chunks(
        graph, chunk_size=args.chunk_size)
    dataset = graph_chunks

    all_discovered_patterns = []

    total_chunks = len(dataset)
    chunk_args = [(chunk_dataset, task, args, idx, total_chunks)
...
def visualize_pattern_graph(pattern, args, count_by_size):
    ...
    try:
                ...
                for key, value in other_attrs.items():
                    if isinstance(value, str):
                        ...
                        if isinstance(value, float):
                            value = f"{value:.2f}" if abs(
                                value) < 1000 else f"{value:.1e}"
        ...
        unique_labels = sorted(set(pattern.nodes[n].get(
            'label', 'unknown') for n in pattern.nodes()))
        label_color_map = {label: plt.cm.Set3(
            i) for i, label in enumerate(unique_labels)}

        unique_edge_types = sorted(
            set(data.get('type', 'default') for u, v, data in pattern.edges(data=True)))
        edge_color_map = {edge_type: plt.cm.tab20(
            i % 20) for i, edge_type in enumerate(unique_edge_types)}
        ...
        if pattern.is_directed():
            arrow_size = 30 if edge_density < 0.3 else (
                20 if edge_density < 0.5 else 15)
        ...
        if edge_density > 0.5:
            font_size = max(
                6, min(9, 150 // (num_nodes + max_attrs_per_node * 5)))
        elif edge_density > 0.3:
            font_size = max(
                7, min(10, 200 // (num_nodes + max_attrs_per_node * 3)))
        else:
            font_size = max(
                8, min(12, 250 // (num_nodes + max_attrs_per_node * 2)))
        ...
        has_anchors = any(pattern.nodes[n].get(
            'anchor', 0) == 1 for n in pattern.nodes())
        ...
        node_types = sorted(set(pattern.nodes[n].get(
            'label', '') for n in pattern.nodes()))
        if any(node_types):
            pattern_info.append('nodes-' + '-'.join(node_types))

        edge_types = sorted(set(pattern.edges[e].get(
            'type', '') for e in pattern.edges()))
        if any(edge_types):
...
def save_and_visualize_all_instances(agent, args, representative_patterns=None):
    try:
        ...
        if representative_patterns:
            logger.info(
                f"Building representative pattern mapping for {len(representative_patterns)} patterns...")
            for rep_pattern in representative_patterns:
                wl = utils.wl_hash(
                    rep_pattern, node_anchored=args.node_anchored)
                representative_map[wl] = rep_pattern
            logger.info(
                f"  Mapped {len(representative_map)} representative patterns")
        ...
        for size in range(args.min_pattern_size, args.max_pattern_size + 1):
            ...
            logger.info(
                f"Size {size}: {len(sorted_patterns)} unique pattern types")
            ...
            for rank, (wl_hash, instances) in enumerate(sorted_patterns[:args.out_batch_size], 1):
                ...
                logger.debug(
                    f"Processing {pattern_key}: {original_count} raw instances")
                ...
                for instance in instances:
                    try:
                        node_ids = frozenset(instance.nodes[n].get(
                            'id', n) for n in instance.nodes())
                        ...
                        logger.warning(
                            f"Error processing instance in {pattern_key}: {e}")
                        continue
                ...
                if visualize_instances and VISUALIZER_AVAILABLE and visualize_all_pattern_instances:
                    try:
                        ...
                        representative_pattern = representative_map.get(
                            wl_hash, None)

                        if representative_pattern:
                            logger.info(
                                f"    Using decoder representative pattern for {pattern_key}")
                        else:
                            logger.warning(
                                f"    No decoder representative found for {pattern_key}, will select from instances")

                        logger.info(
                            f"    Mode: Visualizing representative + {count} instances in subdirectory")
                        ...
                        if success:
                            ...
                            logger.info(
                                f"    ✓ Visualized representative + {count} instances in {pattern_key}/")
                        else:
                            logger.warning(
                                f"    ✗ Visualization failed for {pattern_key}")
                    except Exception as e:
                        logger.error(f"    ✗ Visualization error: {e}")
                elif not visualize_instances:
                    logger.info(
                        f"    Mode: Representatives will be visualized directly in plots/cluster/ (no subdirectories)")
                else:
                    logger.warning(
                        f"    ⚠ Skipping visualization (visualizer not available)")
        ...
        if os.path.exists(pkl_path):
            ...
            logger.info(
                f"✓ PKL file created successfully ({file_size:.1f} KB)")
        else:
        ...
        logger.info(
            f"  Duplicates removed: {total_instances - total_unique_instances}")

        if total_instances > 0:
            dup_rate = (total_instances - total_unique_instances) / \
                total_instances * 100
        ...
        if VISUALIZER_AVAILABLE:
            ...
            logger.info(
                f"  Successfully created: {total_visualizations} files")
...
def pattern_growth(dataset, task, args):
            ...
            for i, graph in enumerate(graphs):
                ...
                for j, node in enumerate(graph.nodes):
                    ...
                    if len(neigh) > 1:
                        ...
                        mapping = {old: new for new,
                                   old in enumerate(subgraph.nodes())}
                        ...
                        for (old_u, old_v), attrs in edge_attrs.items():
                            subgraph.edges[mapping[old_u],
                                           mapping[old_v]].update(attrs)
    ...
    logger.info(
        f"Total time: {elapsed:.2f}s ({int(elapsed)//60}m {int(elapsed)%60}s)")
    ...
    if not visualize_instances and VISUALIZER_AVAILABLE and visualize_pattern_graph_ext:
        ...
        logger.info(
            f"✓ Visualized {successful_visualizations}/{len(out_graphs)} representative patterns")
    elif visualize_instances:
        logger.info(
            "\nSkipping direct representative visualization (representatives already in subdirectories)")
    else:
        logger.warning(
            "⚠ Skipping representative visualization (visualizer not available)")
    ...
    if args.dataset.endswith('.pkl'):
        with open(args.dataset, 'rb') as f:
            ...
            if isinstance(data, (nx.Graph, nx.DiGraph)):
                ...
                logger.info(
                    f"Using NetworkX {graph_type} graph with {graph.number_of_nodes()} nodes and {graph.number_of_edges()} edges")

                sample_edges = list(graph.edges(data=True))[:3]
                if sample_edges:
                    logger.info("Sample edge attributes:")
                    for u, v, attrs in sample_edges:
                        direction_info = attrs.get(
                            'direction', f"{u} -> {v}" if graph.is_directed() else f"{u} -- {v}")
                ...
                logger.info(
                    f"Created {args.graph_type} graph from dict format with {graph.number_of_nodes()} nodes and {graph.number_of_edges()} edges")
            else:
                raise ValueError(
        ...
        # Load a custom graph provided via --graph_pkl_path (used for local workflows)
        if not hasattr(args, 'graph_pkl_path') or not args.graph_pkl_path:
            raise ValueError(
                "For dataset 'custom', please provide --graph_pkl_path pointing to a .pkl file")

        with open(args.graph_pkl_path, 'rb') as f:
            data = pickle.load(f)

        # If the pickle contains a single NetworkX graph
        if isinstance(data, (nx.Graph, nx.DiGraph)):
            graphs = [data]

        # If the pickle contains a list of graphs / items
        elif isinstance(data, list):
            # If it's a list of NetworkX graphs
            if all(isinstance(d, (nx.Graph, nx.DiGraph)) for d in data):
                graphs = data

            # If it's a list of dicts with nodes/edges
            elif all(isinstance(d, dict) and 'nodes' in d and 'edges' in d for d in data):
                graphs = []
                for d in data:
            ...
            # If it's a single element list wrapping a dict with nodes/edges
            else:
                # Try to interpret as a list of edge tuples
                if all(isinstance(e, (list, tuple)) and len(e) >= 2 for e in data):
                    g = nx.DiGraph() if args.graph_type == "directed" else nx.Graph()
                    g.add_edges_from(data)
                    graphs = [g]
                else:
                    raise ValueError(
                        f"Unknown list pickle format in {args.graph_pkl_path}. Expected list of NetworkX graphs, list of dicts with 'nodes'/'edges', or list of edge tuples; got element type {type(data[0]) if data else 'empty list'}")

        # If it's a dict with nodes/edges (single graph)
        elif isinstance(data, dict) and 'nodes' in data and 'edges' in data:
            g = nx.DiGraph() if args.graph_type == "directed" else nx.Graph()
            ...
            raise ValueError(
                f"Unknown pickle format in {args.graph_pkl_path}. Expected NetworkX graph, list of graphs, dict with 'nodes'/'edges', or list of edges, got {type(data)}")

        # Normalize graph directedness and log
        normalized_graphs = []
        for g in graphs:
            if args.graph_type == "directed" and not g.is_directed():
                logger.info("Converting undirected graph to directed...")
                g = g.to_directed()
            elif args.graph_type == "undirected" and g.is_directed():
                logger.info("Converting directed graph to undirected...")
                g = g.to_undirected()
            logger.info(
